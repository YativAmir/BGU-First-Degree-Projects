...
avgQueue1 <- function(time, queueLength, simTime){
  Lavg = 0;
  L = queueLength[1];
  Tnow = time[1];
  Llast = time[1];
...
wedding <- simmer("wedding") %>%
  ...
  add_resource("tortia1", capacity = 4, queue_size =Inf)%>%
  add_resource("tortia2", capacity = 4, queue_size =Inf)%>%
...
dessert_traj<-trajectory("dessert_traj")%>%
  ...
  release("dessert", 1)    # if you got dessert you are allow to go home
...
sushi_traj <- trajectory("sushi_traj")%>%
  simmer::select(resource = c("sushi1","sushi2"),policy = c("shortest-queue-available"),id=4)%>%
...
tortia_traj <- trajectory("tortia_traj")%>%
  simmer::select(resource = c("tortia1","tortia2"),policy = c("shortest-queue-available"),id=5)%>%
  simmer::seize_selected(id=5)%>% 
  simmer::timeout(function()rnorm(1,1.5,0.7))%>%
  simmer::release_selected(id=5)%>%
  timeout(function()rexp(1,0.8))%>%
...
wedding %>%
  add_generator("single", single_traj,distribution=at(SingleSchedule()),mon = 2) %>%
...
or resource='tortia1' or resource='tortia1' or resource='outdoorbar'
...
or resource='banim' or resource='sushi1' or resource='sushi2' or resource='vegie'
or resource='tortia1' resource='tortia2'  or resource='reception1' or resource='reception2')
