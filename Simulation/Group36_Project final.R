...
single_capacity <- function(){ # calculates singles capacity function
...
outdoor_bar_choice <-  function(){ #chances to visit outdoor_bar
  familysize <- get_attribute(wedding,"familysize")
  if(familysize == 1){
    bar_path <-  rdiscrete (1, c(0.2,0.8),c(0,1))
    return(bar_path)
  }
  if(familysize == 2){
    bar_path <-rdiscrete (1, c(0.4,0.6),c(0,1))
    return(bar_path)
  }
  else
    bar_path <-rdiscrete (1, c(0.6,0.4),c(0,1))
  return(bar_path)
  
}

addServiceByAttribute<- function  (path,sname,attName){ # add Service By Attribute
  updatedPath <- seize(path, sname)%>%
...
SizeGroup <-  function(){ # determinate group size function
  SizeGroup <-rdiscrete(1,c(0.33,0.4,0.27),c(3,4,5))
  return (c(SizeGroup))
}
avgQueue <- function(time, queueLength, simTime){ # average queue size function
  Lavg = 0;
  L = queueLength[1];
  Tnow = time[1];
  Llast = time[1];
...
z_value <- function(x){
 ...
 # giving attributes numbers to each station
choose_station<-  function(){
  ...
  # function that decide about a guest going to a station
  ...
  while(Ok==FALSE){
    ...
    if(foodType[1]==1 
       & foodType[2]==1 & foodType[3]==1 
       & foodType[4]==1 & foodType[5]==1 ){
      # ok = TRUE
      break
    }
  }
...
choosing_number <- function(){ # priority function
  while(TRUE){
    u1 <- runif(1,0,1)
    satiety <- 4*u1+1
...
stations_vector <- function(){ #  function that giving starting values for singles and the others
  familysize <- get_attribute(wedding,"familysize")
  if(familysize==1){
    return(c(1,1,0,0,0))
  }
...
food_option <-  function(){ # returning the station that will be visited
  # x <- c(1,2,3,4,5)
...
indoorbar_traj <- trajectory("indoorbar_traj")%>%
  ...
  simmer::timeout(function()rexp(1,2.4))%>%
...
hall_traj <- trajectory("hall_traj")%>%  # after canopy everyone will come here
  ...
  branch(option=function(){rdiscrete (1, c(0.7,0.3),c(1,2))}
         ,continue=c(TRUE,TRUE),meat_meal_traj,vegie_meal_traj)%>%
  synchronize(wait=TRUE,mon_all=TRUE)%>%  # waiting for everyone
  set_prioritization(function()priority())%>%
  seize("dessert", 1, continue = c(FALSE,TRUE) 
        ,post.seize=dessert_traj,reject =dancefloor_traj)%>%
  rollback(amount = 1, times = 1)%>% #try seize again 1 time
  seize("dessert", 1, continue = c(FALSE,FALSE) 
        ,post.seize=dessert_traj,reject =dancefloor_traj)
...
outdoorbar_traj <- trajectory("outdoorbar_traj")%>%
  addService("outdoorbar",function() rexp(1,2))

sushi_traj <- trajectory("sushi_traj")%>%
  simmer::select(resource = c("sushi1","sushi2")
                 ,policy = c("random"),id=4)%>%
...
reception_traj <- trajectory("reception_traj")%>%
  simmer::select(resource = c("reception1","reception2")
                 ,policy = c("shortest-queue-available"),id=3)%>%
  ...
  set_attribute(keys=c("banim","fucacha","vegie","tortia","sushi")
                ,values=function()stations_vector())%>%
  branch(option=function()choose_station(),
         continue=c(TRUE,TRUE,TRUE,TRUE,TRUE)
         ,banim_traj,fucacha_traj,vegie_traj,tortia_traj,sushi_traj)%>%
  branch(option=function()outdoor_bar_choice(),
         continue=TRUE,outdoorbar_traj)%>%
...
#------------------------------------------------ חישוב אורך התור המקסימלי בכל עמדה
x1<-get_mon_resources(wedding)
entitiesData<-get_mon_arrivals(wedding)

weddingData<-get_mon_arrivals(wedding,ongoing = F)%>%
  mutate(waiting_time = end_time - start_time - activity_time)

get_highest_queue <- function(table) {
  grouped_table <- group_by(table, resource)
  highest_queue <- summarize(grouped_table, highest_queue = max(queue))
  return(highest_queue)
}
get_highest_queue(x1)
arrivalDataPerResource <- get_mon_arrivals(wedding, ongoing=TRUE,
                                           per_resource=TRUE)
